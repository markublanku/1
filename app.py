...
def get_moving_average(pair, period=20):
    ...
    if close_prices:
        ...
        print(f"Unable to fetch data for {pair}")
        return None
...
def place_order(pair, amount, side="buy"):
    ...
    if not price:
        print(f"Failed to fetch price for {pair}. Skipping order.")
        return None

    if side == "buy":
        amount_to_buy = round(amount / price, 6)  # Adjust precision as necessary
        order = exchange.create_market_buy_order(pair, amount_to_buy)
        print(f"{datetime.now()}: Bought {amount_to_buy} {pair} at price {price}")
...
def monitor_and_trade():
    ...
    loop_count = 0  # Track loop iterations for testing

    while loop_count < 10:  # Replace 10 with True for infinite loop in production
        ...
        loop_count += 1  # Increment loop count for testing
